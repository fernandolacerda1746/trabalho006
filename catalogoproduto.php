<?php

$produtos = array(
    0=>array(
        'codigo'=>1,
        'nome'=> 'Carro da Quadrilha da Morte',
        'categoria'=>'Veículos',
        'descricao'=>'Proporcionou muitas emoções<br>para nós,<br>espero que seja o mesmo<br>para você',
        'preco'=> 120000.00,
        'foto'=>'Images/Produtos/carro1.jpg',
        'link'=>'produto1.php'
    ),
    1=>array(
        'codigo'=>2,
        'nome'=> 'Foguete Portátil',
        'categoria'=>'Veículos',
        'descricao'=>'Quem precisa de uma moto,<br>quando se tem um foguete.',
        'preco'=> 20000.00,
        'foto'=>'Images/Produtos/foguete1.jpg',
        'link'=>'produto2.php'
    ),
    2=>array(
        'codigo'=>3,
        'nome'=> 'Tônico Acme',
        'categoria'=>'Remédios',
        'descricao'=>'Tônico de origem duvidosa,<br>tome por sua conta em risco.',
        'preco'=> 45.00,
        'foto'=>'Images/Produtos/remedio1.JPG',
        'link'=>'produto3.php'
    ),
    3=>array(
        'codigo'=>4,
        'nome'=> 'Casa Mal Assombrada',
        'categoria'=>'Imóveis',
        'descricao'=>' Parece assustadora,<br>mas os fantasmas<br>são um pouco incompetentes',
        'preco'=> 50000.00,
        'foto'=>'Images/Produtos/casa1.JPG',
        'link'=>'produto4.php'
    ),
    4=>array(
        'codigo'=>5,
        'nome'=> 'Boneca Annabelle',
        'categoria'=>'Brinquedos',
        'descricao'=>'A verdadeira,<br>não a carranca<br>de Hollywood',
        'preco'=> 550.00,
        'foto'=>'Images/Produtos/brinquedo1.JPG',
        'link'=>'produto5.php'
    ),
    5=>array(
        'codigo'=>6,
        'nome'=> 'Fantasia do Pikachu',
        'categoria'=>'Roupas',
        'descricao'=>'Para se divertir,<br>com os coleguinhas<br>da escola',
        'preco'=> 385.15,
        'foto'=>'Images/Produtos/fantasia1.png',
        'link'=>'produto6.php'
    ),
    6=>array(
        'codigo'=>7,
        'nome'=> 'Intel 4004 Personal Computer',
        'categoria'=>'Eletrônicos',
        'descricao'=>'Se um computador desses,<br>levou o homem a lua<br>não precisa de mais<br>para usar um Instagram',
        'preco'=> 4550.00,
        'foto'=>'Images/Produtos/computador1.JPG',
        'link'=>'produto7.php'
    )
);

?>